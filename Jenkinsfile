...
node {
  ...
  sh label: 'Wait for db'           , script: """./integration/wait-for-it/wait-for-it.sh localhost:${DB_CONNECTION}"""
  sh label: 'Wait for moca service' , script: """./integration/wait-for-it/wait-for-it.sh localhost:${SERVICE_CONNECTION}"""
}
